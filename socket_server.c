...
void listarArquivos(SOCKET clientSocket, const char* diretorio) {
    ...
    sprintf_s(searchPath, MAX_PATH, "%s\\*", diretorio);
    
    // Inicia a busca de arquivos
    hFind = FindFirstFileA(searchPath, &findFileData);
    
    if (hFind == INVALID_HANDLE_VALUE) {
        strcpy_s(resposta, DEFAULT_BUFLEN, "Erro ao listar diretório\r\n");
    ...
    strcat_s(resposta, DEFAULT_BUFLEN, "Listagem de arquivos:\r\n");
    ...
    do {
        ...
        if (strcmp(findFileData.cFileName, ".") != 0 && strcmp(findFileData.cFileName, "..") != 0) {
            ...
            strcat_s(resposta, DEFAULT_BUFLEN, linhaArquivo);
        }
    ...
    strcpy_s(resposta, DEFAULT_BUFLEN, "---FIM DA LISTAGEM---\r\n");
    send(clientSocket, resposta, (int)strlen(resposta), 0);
...
void enviarArquivo(SOCKET clientSocket, const char* caminhoArquivo) {
    ...
    if (fopen_s(&arquivo, caminhoArquivo, "rb") != 0) {
        sprintf_s(buffer, DEFAULT_BUFLEN, "ERRO: Não foi possível abrir o arquivo %s\r\n", caminhoArquivo);
    ...
    sprintf_s(buffer, DEFAULT_BUFLEN, "TAMANHO:%u\r\n", tamanhoArquivo);
    send(clientSocket, buffer, (int)strlen(buffer), 0);
    ...
    strcpy_s(buffer, DEFAULT_BUFLEN, "---FIM DO ARQUIVO---\r\n");
    send(clientSocket, buffer, (int)strlen(buffer), 0);
}
...
void receberArquivo(SOCKET clientSocket, const char* caminhoArquivo, DWORD tamanhoArquivo) {
    ...
    if (fopen_s(&arquivo, caminhoArquivo, "wb") != 0) {
        sprintf_s(buffer, DEFAULT_BUFLEN, "ERRO: Não foi possível criar o arquivo %s\r\n", caminhoArquivo);
        send(clientSocket, buffer, (int)strlen(buffer), 0);
        return;
    }
    
    // Envia confirmação para iniciar a transferência
    strcpy_s(buffer, DEFAULT_BUFLEN, "PRONTO_PARA_RECEBER\r\n");
    send(clientSocket, buffer, (int)strlen(buffer), 0);
    ...
    strcpy_s(buffer, DEFAULT_BUFLEN, "ARQUIVO_RECEBIDO\r\n");
    send(clientSocket, buffer, (int)strlen(buffer), 0);
}
...
void removerArquivo(SOCKET clientSocket, const char* caminhoArquivo) {
    ...
    if (DeleteFileA(caminhoArquivo)) {
        sprintf_s(buffer, DEFAULT_BUFLEN, "Arquivo %s removido com sucesso\r\n", caminhoArquivo);
    } else {
        sprintf_s(buffer, DEFAULT_BUFLEN, "ERRO: Não foi possível remover o arquivo %s (Código: %d)\r\n", 
...
void processarComando(SOCKET clientSocket, ComandoArquivo* comando) {
    switch (comando->comando) {
        ...
        default:
            ...
            {
                char resposta[100];
                sprintf_s(resposta, 100, "Comando desconhecido: %c\r\n", comando->comando);
                send(clientSocket, resposta, (int)strlen(resposta), 0);
            }
            break;
    }
...
int main(int argc, char* argv[]) {
    ...
    printf("Servidor iniciado. Aguardando conexões na porta %s...\n", DEFAULT_PORT);
